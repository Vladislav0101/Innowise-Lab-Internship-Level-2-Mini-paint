"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var firebase_1 = require("firebase");
var main_1 = require("@/main");
var helpFunction_1 = require("@/utils/helpFunction");
var state = {
    userAccountInfo: null,
    userAvatar: null
};
var getters = {
    userAccountInfo: function (state) {
        return state.userAccountInfo;
    },
    userAvatar: function (state) {
        return state.userAvatar;
    }
};
var mutations = {
    setUserAccountInfo: function (state, userInfo) {
        state.userAccountInfo = userInfo;
    },
    setUserAvatar: function (state, url) {
        state.userAvatar = url;
    }
};
var actions = {
    checkEqualVersion: function (_a) {
        var getters = _a.getters, commit = _a.commit, dispatch = _a.dispatch;
        return __awaiter(this, void 0, void 0, function () {
            var emailToDB;
            return __generator(this, function (_b) {
                emailToDB = helpFunction_1.stringToDBFormat(getters.email);
                firebase_1["default"]
                    .database()
                    .ref(emailToDB + "/version")
                    .on("value", function (res) {
                    var result = res.val();
                    if (!result || result.version !== getters.version) {
                        dispatch("setVersionOnDB");
                    }
                    commit("setIsVersionsMatch", result.version === getters.version);
                    if (!result.checkedVersion) {
                        main_1.eventBus.$emit("isNeedToLearningPath");
                    }
                });
                commit("setFeatures");
                return [2 /*return*/];
            });
        });
    },
    setVersionOnDB: function (_a, value) {
        var getters = _a.getters;
        return __awaiter(this, void 0, void 0, function () {
            var emailToDB;
            return __generator(this, function (_b) {
                emailToDB = helpFunction_1.stringToDBFormat(getters.email);
                firebase_1["default"]
                    .database()
                    .ref(emailToDB + "/version")
                    .set({
                    version: getters.version,
                    checkedVersion: value ? value : false
                });
                return [2 /*return*/];
            });
        });
    },
    setUserInfo: function (_a, userInfo) {
        var dispatch = _a.dispatch, getters = _a.getters;
        var emailToDB = helpFunction_1.stringToDBFormat(getters.email);
        firebase_1["default"]
            .database()
            .ref(emailToDB + "/userInfo")
            .set(userInfo);
        dispatch("getUserInfo");
    },
    setUserAvatar: function (_a, _b) {
        var getters = _a.getters, dispatch = _a.dispatch, commit = _a.commit;
        var img = _b.img;
        var storageRef = firebase_1["default"].storage().ref("avatars/");
        var emailToDB = helpFunction_1.stringToDBFormat(getters.email);
        storageRef
            .child(getters.email + ".jpeg")
            .putString(img, "data_url")
            .then(function () {
            firebase_1["default"]
                .database()
                .ref(emailToDB + "/isAvatar")
                .set(true);
        });
        commit("setUsersAvatars", { email: getters.email, img: img });
    },
    getUserInfo: function (_a) {
        var getters = _a.getters, commit = _a.commit;
        var emailToDB = helpFunction_1.stringToDBFormat(getters.email);
        firebase_1["default"]
            .database()
            .ref(emailToDB + "/userInfo")
            .on("value", function (res) {
            commit("setUserAccountInfo", res.val());
        });
    }
};
exports["default"] = {
    state: state,
    getters: getters,
    mutations: mutations,
    actions: actions
};
